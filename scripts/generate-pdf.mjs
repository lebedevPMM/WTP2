...
const content = {
  en: {
    ...
    benefits: {
      ...
      items: [
        { title: 'Client Protection', text: 'Your client relationships remain yours. Documented under your name, contractually protected, no direct contact without permission.' },
  ...
  ru: {
    tag: 'Для налоговых консультантов и управляющих активами',
    headline: 'Надежный execution-партнер в ОАЭ.<br>Вы ведете клиента — мы делаем все на месте.',
    subheadline: 'Регистрация, счета, визы, комплаенс. Возможность открытия счета проверяется до начала работы.',
    cta: 'Отправить пилотный кейс',
    benefits: {
      title: 'Почему партнеры выбирают WTP',
      label: 'Преимущества',
      items: [
        { title: 'Защита клиентов', text: 'Ваши клиентские отношения остаются вашими. Закрепление клиента за партнером, договорная защита, без прямого контакта.' },
        { title: 'Осознанный подход к рискам', text: 'Мы отказываем профилям, не соответствующим критериям. Три уровня: Зеленый, Желтый, Красный. Отклоняем ~30% кейсов.' },
        { title: 'Прозрачность', text: 'Еженедельные обновления статуса, контрольные точки, документированный процесс. Никаких сюрпризов.' },
        { title: 'Банковское обслуживание в приоритете', text: 'Начинаем с требований банка, а не с документов. Компания зарегистрирована, а счет не открыть — мы это предотвращаем.' },
      ],
    },
    process: {
      ...
      steps: [
        ...
        { num: '02', title: 'Банковский сценарий', text: 'Подбор банка под профиль клиента. Включен в pre-screen.' },
    ...
    risk: {
      ...
      label: 'Фильтрация',
      rows: [
        { color: 'green', status: 'Зеленый — принимаем', text: 'Прозрачная бизнес-логика, готовность документов, отсутствие тревожных сигналов, реалистичные ожидания.' },
        { color: 'yellow', status: 'Желтый — условно', text: 'Повышенный риск, сложная структура, нестандартные операции. Усиленная проверка и контроли.' },
        { color: 'red', status: 'Красный — отклоняем', text: 'Санкции, токсичная экспозиция, отсутствие документов, давление на обход комплаенс. Отклоняем с объяснением.' },
      ],
    },
    products: {
      title: 'Услуги',
      label: 'Спектр',
      items: [
        { pill: 'Ключевая', name: 'Корпоративные счета', desc: 'Открытие счетов, подбор банка, подготовка документов об источнике средств (SOF)' },
        { pill: 'Персональный', name: 'Личные счета', desc: 'Премиальное банковское обслуживание, инвестиции' },
        { pill: 'Операции', name: 'Регистрация компаний', desc: 'Свободная зона, материковая юрисдикция, лицензии' },
        ...
        { pill: 'Наследие', name: 'Активы и защита', desc: 'Завещания, фонды, семейный офис' },
      ],
    },
    footer: {
...
function buildHTML(lang) {
  ...
  const parts = [
    ...
    '    <a href="mailto:' + c.footer.email + '" class="cta" style="text-decoration:none;color:var(--bg)">' + c.cta + '</a>',
    '  </div>',
    '</div>',
    '',
    '<div class="section">',
